...
goEDMS uses database migrations to manage schema changes. Migrations run automatically when the application starts and connects to the database.
...
When you start goEDMS, it automatically:
...
psql -U goedms -d goedms -f database/migrations/000002_add_fulltext_search.up.sql
```
...
```bash
# Backup first!
pg_dump -U goedms goedms > backup.sql

# Drop and recreate
psql -U postgres -c "DROP DATABASE goedms;"
psql -U postgres -c "CREATE DATABASE goedms OWNER goedms;"
...
| Manually apply migration | `psql -U goedms -d goedms -f database/migrations/XXX.up.sql` |
