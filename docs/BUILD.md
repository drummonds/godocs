...
This document describes how to build goEDMS from source.
...
```bash
mkdir -p build
go build -o build/goedms main.go
```
...
GOOS=js GOARCH=wasm go build \
  -ldflags="-X 'github.com/drummonds/goEDMS/webapp.Version=$VERSION' \
            -X 'github.com/drummonds/goEDMS/webapp.BuildDate=$BUILD_DATE'" \
  -o web/app.wasm \
  ./cmd/webapp
```
...
3. **Verify version:**
   ...
   ./build/goedms --version  # (if version flag is implemented)
   ```
