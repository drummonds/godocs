...
This document describes the ephemeral PostgreSQL implementation in godocs, based on the approach from [Michael Stapelberg's blog post](https://michael.stapelberg.ch/posts/2024-11-19-testing-with-go-and-postgresql-ephemeral-dbs/).

## Overview

godocs now supports ephemeral PostgreSQL databases for development and testing. This approach replaces the previous `embedded-postgres` library with `postgrestest`, providing:
...
When you run godocs with PostgreSQL configured but no connection string provided, it automatically:
